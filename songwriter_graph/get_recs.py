...
def get_recs(song_id, k=10, cos_sim_mat=cos_sim_mat_g):
    ...
    try:
        ...
        return pd.DataFrame(
            ...
            index=["Song Name", "Artist", "Song ID", "Similarity"],
        ).T
    except:
        print(
            """No results available for that id. 
...
def get_recs_da(
    ...
    try:
        top_songs_feat, top_songs_feat_sim, artist_songs = feat_sim_da(
            song_id, k, main_song_list, cos_sim_mat
        )
        recs = [
            [song_id_name[song_cosine_idr[x]] for x in top_songs_feat],
            [song_id_artist[song_cosine_idr[x]] for x in top_songs_feat],
        ...
        recs = pd.DataFrame(
            recs, index=["Song Name", "Artist", "Song ID", "Similarity"]
        ).T
