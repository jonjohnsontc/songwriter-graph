...
from notebooks.library.utils import (
    find_latest_file,
    find_latest_file_s3,
    load_dd,
    load_df,
...
def get_euclidean(modeling_ready_ddf):
    ...
    with fs.open(
        f"s3://swg-bucket/processed/distances/distances_{TIME_AT_RUN}.pkl",
        "wb",
    ) as f:
        pkl.dump(distances, f)
    with fs.open(
        f"s3://swg-bucket/processed/indices/indices_{TIME_AT_RUN}.pkl", "wb"
    ) as f:
        pkl.dump(indices, f)
...
def create_model_df(labels, matrix, type_of_file):
    ...
    fs = s3fs.S3FileSystem()
    with fs.open(
        f"s3://swg_bucket/processed/{type_of_file}/{type_of_file}_df_{TIME_AT_RUN}.csv",
        "wb",
    ) as f:
...
def parse_args():
    parser = argparse.ArgumentParser(
        "Description: Generate models based on songwriter data passed through"
    )
    parser.add_argument(
        "path", help="Location of the modeling ready songwriter data", type=str
    )
