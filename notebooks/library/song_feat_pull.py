...
def rm_unnecessary_entries(song_features_json_fp):
    ...
    song_features_ddf = dd.read_json(song_features_json_fp,
                                        orient = 'index')
    clean_song_features_ddf = song_features_ddf.drop(axis = 1,
                                                    labels = [
                                                    'id',
                                                    'track_href',
                                                    'uri',
...
def main():
    ...
    song_features_path = glob.glob('data/interim/song_features/*') #TODO: Edit into something that finds the newest file
